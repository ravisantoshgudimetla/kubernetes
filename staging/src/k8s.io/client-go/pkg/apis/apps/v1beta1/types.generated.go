	...
	for yyj3 := 0; ; yyj3++ {
		...
		case "volumeClaimTemplates":
			if r.TryDecodeAsNil() {
				...
				yyv10 := &x.VolumeClaimTemplates
				yym11 := z.DecBinary()
				_ = yym11
				if false {
				} else {
					h.decSlicev1_PersistentVolumeClaim((*[]pkg3_v1.PersistentVolumeClaim)(yyv10), d)
				}
			}
		case "serviceName":
			if r.TryDecodeAsNil() {
...
func (x *StatefulSetSpec) codecDecodeSelfFromArray(l int, d *codec1978.Decoder) {
	...
	if r.TryDecodeAsNil() {
		...
		yyv25 := &x.VolumeClaimTemplates
		yym26 := z.DecBinary()
		_ = yym26
		if false {
		} else {
			h.decSlicev1_PersistentVolumeClaim((*[]pkg3_v1.PersistentVolumeClaim)(yyv25), d)
		}
	}
	yyj18++
	if yyhl18 {
